...
public class ApkViewDumper {
    ...
    public static void main(String[] args) {
        ...
        String activityClass = "com.example.hello.HelloActivity";
        if (args.length > 0 && args[0].length() > 0) {
            activityClass = args[0];
        }

        // Derive package name from activity class (everything before last dot-segment)
        String packageName = activityClass;
        int lastDot = activityClass.lastIndexOf('.');
        if (lastDot > 0) {
            packageName = activityClass.substring(0, lastDot);
        }
